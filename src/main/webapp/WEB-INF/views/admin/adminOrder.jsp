		...
		<table>
			...
			<c:forEach items="${AdminOrderListVO}" var="a">
				<tr>
					<td>${a.order_date}</td>
					<td>
					<a href="${pageContext.request.contextPath}/adminOrderDetail?ORDERNUM=${a.orderNum}">${a.orderNum}</a>
					</td>
					<td>${a.memID}</td>
					<td>${a.order_method}</td>
					<td>${a.order_price}</td>
					<td>${a.order_state}</td>
					<td><input type="button" onclick="location.href='${pageContext.request.contextPath}/adminOrderList'"value="취소"></td>	
				</tr>
			</c:forEach>
		</table>
